...
//
// Replica exatamente o que `public/upload.html` do site faz (ver
// CONTRATO_UPLOAD.md): POST /api/tracks/upload com header
// `Authorization: Bearer <access_token>`, body = FormData com:
//   - campos de texto: title, genre, bpm, music_key, daw, price_eur,
//     description, has_project_file, cover_emoji, metadata (JSON)
//   - `file`  = Extended Mix master WAV (campo principal)
//   - `cover` = imagem de capa (opcional)
//   - `xf_<fkey>` = restantes arquivos (mixdowns, instrumentais, MP3, stems...)
//
// Os arquivos sao transmitidos em streaming direto do disco (nao carregamos
// 800MB em memoria). A resposta e devolvida crua ao frontend.
...
pub struct UploadResult {
    pub ok: bool,
    /// status HTTP (0 se nem chegou a responder)
    pub status: u16,
    /// mensagem de sucesso/erro extraida do JSON da resposta
    pub message: String,
    /// avisos do backend (ex: arquivo grande nao guardado)
    pub warnings: Vec<String>,
    /// id da track criada, se houver
...
/// Envia a track direto para /api/tracks/upload (publicacao imediata, legado).
pub async fn upload(app: AppHandle, payload: UploadPayload) -> Result<UploadResult, String> {
    let client = prepare(&app, &payload.files).await?;
    let url = format!("{}/api/tracks/upload", APP_BASE_URL);
    ...
    if status.is_success() && body.get("success").and_then(|s| s.as_bool()).unwrap_or(false) {
        let message = body
            .get("message")
            .and_then(|m| m.as_str())
            .unwrap_or("Track uploaded — pending review.")
            .to_string();
        let track_id = body
            .get("track")
            .and_then(|t| t.get("id"))
            .map(|id| id.to_string().trim_matches('"').to_string());
        progress(&app, "done", message.clone());
        Ok(UploadResult { ok: true, status: status.as_u16(), message, warnings, track_id })
    } else {
        let message = body
            .get("error")
            .and_then(|e| e.as_str())
        ...
        Ok(UploadResult { ok: false, status: status.as_u16(), message, warnings, track_id: None })
    }
}
